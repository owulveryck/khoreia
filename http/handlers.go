...
func TaskList(w http.ResponseWriter, r *http.Request) {
	...
	for id, task := range tasks {
		c := Content{
			id,
			(task).Name,
			choreography.States[(task).State],
			time.Time{},
			time.Time{},
		}
		co[i] = c
...
func TaskCreate(w http.ResponseWriter, r *http.Request) {
	...
	uuid := uuid()
	exe := choreography.ExecutorBackend{
		Name:        "self",
		Url:         "https://127.0.0.1:8585/v1",
		Certificate: "choreography.pem",
		Key:         "choreography_key.pem",
		CACert:      "executor.pem",
		Ping:        "/ping",
		Client:      nil,
	}
	...
	go v.Run([]choreography.ExecutorBackend{exe})
